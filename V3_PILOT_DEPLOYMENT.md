...
- Set `APP_MODE=demo`. The server mounts only `/health`, `/api/v1/health`, and `/api/v1/demo/*` plus the public React application.
- Staff auth, member, dashboard, reports, admin, and webhook routes are not mounted in demo mode.
...
3. Confirm the success counts, open the dashboard, search the roster, and open the booking CTA.
4. Upload XLSX and the title-row CSV; confirm both load.
5. Confirm `.numbers`, oversized, over-5,000-row, and over-100-column files show friendly errors.
6. Confirm a real/non-demo club ID returns `404` from the public overview endpoint.
7. Temporarily expire a demo in the sandbox, run the cleanup job, and verify its jobs, prospect, club, memberships, and people are deleted.
8. Verify the Wedgewood App Runner service and production database have no V3 migration or configuration changes.
